"""SupContrast ResNet
Adopted from: https://github.com/deeplearning-wisc/knn-ood/blob/master/models/resnet_supcon.py
Originate from: https://github.com/HobbitLong/SupContrast/blob/master/networks/resnet_big.py
"""
...
class SupConResNet(nn.Module):
    ...
    def forward(self, x, proj_normalize=False):
        feat = self.encoder(x)
        # feat = F.normalize(self.head(feat), dim=1)
        feat_proj = self.head(feat)
        if proj_normalize:
            feat_proj = F.normalize(feat_proj, dim=1)
        return feat_proj, feat

    def feature_list(self, x, proj_normalize=False):
        feat_out, feat_list = self.encoder.feature_list(x)
        feat_proj = self.head(feat_out)
        if proj_normalize:
            feat_proj = F.normalize(feat_proj, dim=1)
        # return self.fc(feat_out), feat_list
        return feat_proj, feat_list
