...
def mktrainval(args):
    ...
    train_set = id_train_loader.dataset
    valid_set = id_test_loader.dataset
    train_loader = id_train_loader
    valid_loader = id_test_loader

    return train_set, valid_set, train_loader, valid_loader



def tune_mahalanobis_hyperparams(args, model, num_classes, train_loader, val_loader, save_dir):
    ...
    for magnitude in [0.0, 0.01, 0.005, 0.002, 0.0014, 0.001, 0.0005]:
        ...
        total = 0
        for data_index in tqdm(range(int(np.floor(train_lr_data.size(0) / args.batch_size)) + 1)):
            if total >= 2*m:
                break
    ...
    ###################################Out-of-Distributions#####################################
        ...
        _, _, fpr = get_measures(all_confidence_scores_in, all_confidence_scores_out)
...
def main(args):
    ...
    model = get_model(arch=args.arch, args=args, load_file=args.load_file, strict=True)

    print("Moving model onto all GPUs")
    model = torch.nn.DataParallel(model)
    model = model.cuda()

    save_dir = os.path.join(args.save_folder, args.id_dset)
    save_dir = os.path.join(save_dir, f"{args.arch}")
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    print('Tuning hyper-parameters...')
    sample_mean, precision, best_regressor, best_magnitude \
        = tune_mahalanobis_hyperparams(args, model, val_set.cls_num, train_loader, val_loader, save_dir)

    print('saving results...')
    # pdb.set_trace()
    sample_mean = [x.cpu().numpy() for x in sample_mean]
    precision = [x.cpu().numpy() for x in precision]
    np.save(os.path.join(save_dir, 'maha_tune_results'),
