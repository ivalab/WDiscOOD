...
def sample_estimator(model, num_classes, feature_list, train_loader):
    ...
    # CYY: use a subset of data
    # fit_data_num = 200000     # if None, will use all data, which will most likely cause memory issue on large dataset
    fit_data_num = None     # if None, will use all data, which will most likely cause memory issue on large dataset
    batch_total_num = int(fit_data_num / train_loader.batch_size) if fit_data_num is not None else None
    total = batch_total_num if batch_total_num is not None else len(train_loader)
    for num_batch, (data, target) in tqdm(enumerate(train_loader), total=total):
        torch.cuda.synchronize()
        # if num_batch % 100 == 0:
            # print('{} batches processed...'.format(num_batch))
        # let's try to use a subset of data
        if batch_total_num is not None and num_batch > batch_total_num:
            print(f"Processed over {fit_data_num} data. Exiting...")
            break
